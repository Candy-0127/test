...
null}">update</c:when><c:otherwise>add</c:otherwise></c:choose>">
			...
			<input type="text" id="username" name="username" value="<c:out
value="${userToEdit.username}"/>" <c:if test="${userToEdit != null}">readonly</c:if> required>
			<label for="password">パスワード:</label>
			<input type="password" id="password" name="password" <c:if test="${userToEdit ==
null}">required</c:if>>
			<c:if test="${userToEdit != null}">
...
合は別途操作してください。</p>
			...
			<select id="role" name="role" required>
				<option value="employee" <c:if test="${userToEdit.role ==
'employee'}">selected</c:if>>従業員</option>
				<option value="admin" <c:if test="${userToEdit.role ==
...
secondary" onclick="return confirm('本当にパスワードをリセットしますか？（デフォルトパスワー
ド: password）');">
					</form>
				</c:if>
			</div>
				...
				<tr>
					...
					<td>
						<form action="users" method="post" style="display:inline;">
							...
							<input type="submit" value="<c:choose><c:when
test="${u.enabled}">無効化</c:when><c:otherwise>有効化</c:otherwise></c:choose>" class="button
<c:choose><c:when
